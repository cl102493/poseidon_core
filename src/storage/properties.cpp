...
static std::regex
    dtime_expr("^((19[7-9][0-9])|(20[0-9][0-9]))-" // YYYY-MM-DDTHH:MM:SS:ZZZZ
               "(0[1-9]|1[012])-"
               "(0[1-9]|[12][0-9]|3[01])T"
               "([01][0-9]|2[0-3]):"
               "([0-5][0-9]):"
...
p_item::p_item(dcode_t k, p_item::p_typecode tc, const std::any &v)
    : key_(k), flags_(0) {
  switch (tc) {
  case p_int:
    ...
    break;
  case p_double:
    set<double>(std::any_cast<double>(v));
    break;
  case p_dcode:
    set<dcode_t>(std::any_cast<dcode_t>(v));
    break;
  case p_uint64:
  ...
  case p_ptime:
  case p_date:
    set<ptime>(std::any_cast<ptime>(v));
    break;
  default:
    break;
  }
...
p_item::p_item(dcode_t k, p_item::p_typecode tc, const std::string &v,
               dict_ptr &dict)
    : key_(k), flags_(0) {
  ...
  case p_int:
    ...
    break;
  case p_double:
    set<double>(std::stod(v));
    break;
  case p_dcode:
    set<dcode_t>(dict->insert(v));
    break;
  case p_uint64:
    set<uint64_t>(std::stoull(v));
    break;
  case p_ptime:
    set<ptime>(boost::posix_time::time_from_string([&]() {
      std::string s = v;
      s[s.find("T")] = ' ';
    ...
    break;
  case p_date:
    set<ptime>(boost::posix_time::ptime(
...
p_item::p_item(dcode_t k, boost::posix_time::ptime v) : key_(k), flags_(0) {
  set<ptime>(v);
}
...
std::ostream &operator<<(std::ostream &os, const p_item &pi) {
  os << "(" << static_cast<unsigned int>(pi.flags_) << "|" << pi.key_ << "|"
     << pi.get_raw() << ")";
  return os;
}
