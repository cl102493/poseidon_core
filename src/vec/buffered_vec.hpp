 ...
 */
template <typename T, int num_records> struct bchunk {
  T data_[num_records]; // the array of data
  std::bitset<num_records>
 ...
 * (starting at 0). There are two ways to add element to buffered_vec: either
 * via store_at() to store an element at a given position or via append() where
 * the element is added to the end of the vector. For iterating over
 * buffered_vec also two methods are available: begin()/end() are used for an
 * element-wise iteration while range allows to specifiy a start and end chunk
 * for the iteration.
 */
template <typename T> class buffered_vec {
  static constexpr auto num_entries =
      static_cast<int>((DEFAULT_BCHUNK_SIZE - 13) / (sizeof(T) + 1 / 8.0));
...
public:
  ...
  class iter {
  public:
    iter(buffered_vec &bv, paged_file::page_id pid, paged_file::page_id num,
         offset_t p = 0)
  ...
  struct range_iter {
    range_iter(buffered_vec &v, paged_file::page_id first,
               paged_file::page_id last, std::size_t pos = 0)
        : bvec_(v), range_(first, last), current_pid_(first), cptr_(nullptr),
          pos_(pos) {
    ...
    range_iter &operator++() {
      do {
        ...
        // make sure, cptr_[pos_] is valid
      } while (current_pid_ <= range_.second && cptr_ && !cptr_->is_used(pos_));
      return *this;
    }
  ...
  private:
    ...
    std::pair<paged_file::page_id, paged_file::page_id>
        range_;                       // range of chunks to visit
    paged_file::page_id current_pid_; // position of the current chunk
                                      // (initially range_.first)
    bchunk_ptr cptr_; // pointer to the current chunk
    offset_t pos_;    // position within the current chunk
  };
  ...
  range_iter *range_ptr(std::size_t first_chunk, std::size_t last_chunk,
                        std::size_t start_pos = 0) {
    return new range_iter(*this, first_chunk + 1, last_chunk + 1, start_pos);
  }

  void pin(paged_file::page_id pid) { bpool_.pin_page(pid | file_mask_); }

  void unpin(paged_file::page_id pid) { bpool_.unpin_page(pid | file_mask_); }
   ...
   */
  std::pair<offset_t, T *>
  append(T &&o, std::function<void(offset_t)> callback = nullptr) {
   ...
   */
  std::pair<offset_t, T *>
  store(T &&o, std::function<void(offset_t)> callback = nullptr) {
   ...
   * offset is relative to the begining of the buffered_vec. The first element
   * of the buffered_vec has always an offset=0.
   */
  ...
  void resize(int nchunks) {
    ...
    for (auto i = 0; i < nchunks; i++) {
      ...
      chk->slots_.reset();
      chk->first_ = 0;
      spdlog::debug("resize -> {} add {}", pg.second,
                    pg.second * elems_per_chunk_);
      add_to_free_list(pg.second * elems_per_chunk_ -
                       1); // calculate a record offset
...
private:
  ...
  bchunk_ptr find_chunk(offset_t idx, bool modify = false) const {
    ...
    // std::cout << "find_chunk: " << page_id << ", addr=" <<
    // (uint64_t)pg->payload << std::endl;

    if (modify) {
      spdlog::debug("buffered_vec::find_chunk page #{} marked as dirty",
                    page_id | file_mask_);
  ...
  bchunk_ptr load_chunk(paged_file::page_id pid, bool modify = false) const {
    ...
    if (modify) {
      spdlog::debug(
          "buffered_vec::load_chunk page #{} marked as dirty in file {}",
          pid | file_mask_, file_id_);
  ...
  bchunk_ptr get_last_chunk(bool modify = false) const {
    ...
    if (modify && pg.first) {
      spdlog::debug(
          "buffered_vec::get_last_chunk page #{} marked as dirty in file {}",
          pg.second | file_mask_, file_id_);
      bpool_.mark_dirty(pg.second | file_mask_);
    }
    return pg.first != nullptr ? reinterpret_cast<bchunk_ptr>(pg.first->payload)
                               : nullptr;
  }
  ...
  std::bitset<65536>
      freelist_; // a bitset representing chunks containing available slots (1 =
                 // slots available, 0 = not)
