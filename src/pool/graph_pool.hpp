...
public:
  ...
  /**
   * Open an existing pool. If a pool was created as poolset via 'pmempool
   * create' the pool should be opened with init = true.
   */
  static graph_pool_ptr open(const std::string &path, bool init = false);

  /**
   * Destroys the given pool. Works only with single pool but not with a
   * poolset.
   */
